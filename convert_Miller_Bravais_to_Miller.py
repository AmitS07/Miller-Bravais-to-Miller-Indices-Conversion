...
basal_plane = np.array([[0,0,0,1],[0,0,0,1],[0,0,0,1]]) #3-times mentioned to accomodate the 3-slip direction
...
basal_cartFrame = [np.matmul(L_hcp_rcp,ii) for ii in basal_miller] # Miller Indices in Cartesian form, for Basal slip-planes
prism_cartFrame = [np.matmul(L_hcp_rcp, ii) for ii in prism_miller] # Miller Indices in Cartesian form, for Prismatic slip-planes
pyra_cartFrame = [np.matmul(L_hcp_rcp, ii) for ii in pyra_miller] # Miller Indices in Cartesian form, for Pyramidal slip-planes
## Normalizing Planes
...
#%% Section-4: Testing whether Direction lies in Planes or not i.e. hu +kv +lw = 0, i.e. angle between slip plane-normal to slip direction should be 90, i.e. cos90 = 0
...
## Printing angles
for angle in angle_bw_plane_dir:
    print(f"{angle:.3f}")
